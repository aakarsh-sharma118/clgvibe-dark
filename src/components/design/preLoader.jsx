      ...
      {isLoading ? (
        <div className="loading-page absolute top-0 left-0 h-full w-full flex flex-col items-center justify-center gap-6 bg-background-gradient">
          <svg
            ...
            viewBox="0 0 640 512"
            className="h-36 w-36 stroke-white fill-transparent stroke-8 animate-draw"
          >
          ...
          <div className="name-container h-8 overflow-hidden">
            <div className="logo-name text-white text-lg tracking-wide ml-5 font-bold">
              ClgVibe
            </div>
          </div>
        </div>
      ) : (
        <div className="content !opacity-100 !block">{children}</div>
      )}
    </>
  );
};
